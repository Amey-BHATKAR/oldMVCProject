...
function finishAjaxOther(id, response) {
 $('#wait_2').hide();
...
function error(ValIn,ValOrig)
{
    ...
    if (x1 >= (x2/2))
    {
    	...
    	{
    		alert('Entered value should not be greater than twice the current value.');
    		window.location = "/forecast/newMVCProj/views/salesViews/mainPage.php";
    	}
     
    }
    else
    {
    	alert('Entered value should not be less than half of the current value.');
    	window.location = "/forecast/newMVCProj/views/salesViews/mainPage.php";
    }
...
<link href="/forecast/newMVCProj/css/bootstrapmain.min.css" rel="stylesheet" media="screen">
	...
	<form method = "post" action = "/forecast/newMVCProj/views/salesViews/mainTTLPage.php">
 		...
 		<input type="submit" name="Submit" value="Select from Dev Products" style="font-weight: normal; vertical-align: middle; font-size: 20; height: 30px; width: 250px"/>
 	</form>
	<?php
    ...
    <?php //if($showProd) { drop_1($custName); } else{}
							...
							<tr>
								...
								<?php 
											...
											else
											{
												...
												<input type="hidden" name="forecastValue[0]" value = "<?php echo ($forecastValues[0]);?>"/>
												<input type="hidden" name="forecastValue[1]" value = "<?php echo ($forecastValues[1]);?>"/>
												<td id = "cellSix" align = center><input id = "input" type="text" onchange = "error(this.value,<?php echo ($forecastValues[2]);?>);" name="forecastValue[2]" value = "<?php echo ($forecastValues[2]);?>"/></td>
												<td id = "cellSix" align = center><input id = "input" type="text" onchange = "error(this.value,<?php echo ($forecastValues[3]);?>);" name="forecastValue[3]" value = "<?php echo ($forecastValues[3]);?>"/></td>
												<?php for($i = 4; $i < 26; $i++)
												{
						...
						<table>
							<tr>
								...
								if($current['Customer_Name'] == "ASCENDEO UK IRELAND/CENTROSUS MOBIL")
 								...
 								?>
								<td><img src = "/forecast/newMVCProj/controllers/pictures/<?php echo $custTemp."_".$current['Product_Name'];?>.jpeg"></img></td>
